...
class StockPredictor:
    ...
    def train_model(self, X_train, y_train):
        ...
        print(f"GRU Layer 1 Units: {best_hp.get('units_1')}")
        print(f"Dropout 1: {best_hp.get('dropout_1')}")
        print(f"GRU Layer 2 Units: {best_hp.get('units_2')}")
    ...
    def visualize_results(self, y_test_actual, y_pred_actual):
        ...
        fig.update_layout(title=f'{self.ticker} Stock Analysis & 7-Day GRU Prediction', height=850, xaxis_rangeslider_visible=False,
                          showlegend=True, yaxis2=dict(overlaying='y', side='right', showgrid=False))
        fig.show()

    # Run everything
    def run(self, ticker="MSFT"):
        try:
